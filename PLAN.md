...
All six resolved in the "fix open topics" batch.

- **Server-side overlap check on `POST`/`PUT /periods`.** ✅ Resolved (`3ffd0b0`): both reject overlaps with `409` via `hasOverlap` (PUT excludes the edited row); the `add` overlap → exit-1 path now works end-to-end.
- **Consolidate the vacation-type runtime lists.** ✅ Resolved (`0912e8b` + `492e199`): `parseImportCSV` (in `csv.ts`) and `VacationModal` both use the canonical `VACATION_TYPES` from `src/types.ts`; no per-module copies remain.
- **Keep browser (DOM) code out of the server's import graph.** ✅ Resolved (`6e81280`): `downloadSingleICS` moved to `src/utils/icsDownload.ts`, so `ics.ts` is DOM-free (`parseImportCSV` had already moved to `csv.ts`). Added `typecheck:server` + a combined `typecheck` and wired it into CI; `tsc -p tsconfig.server.json` is green.
- **`list`'s "Days" column is a calendar-day span, not working days.** ✅ Resolved (`383bb58`): `GET /periods` returns a server-computed `workDays` per period (ADR 0002, from the configured Bundesland); `list` renders it, falling back to the calendar span only when absent.
- **Partial `migrate` prints its summary to stderr (non-json).** ✅ Resolved (`cd03c16`): `runMigrate` returns `{ output, ok }`; the action prints to stdout and sets exit 1 on a partial import. Replaced the `cacafb0` `UsageError.json` mechanism.
- **CLI CSV export has no BOM.** ✅ Resolved (`9e3cbbe`): opt-in `export --bom` prepends a UTF-8 BOM to CSV output for Excel.
