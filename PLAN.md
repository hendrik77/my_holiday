...
- **Keep browser (DOM) code out of the server's import graph.** T1.4 has `server/routes.ts` import `parseImportCSV` from `src/utils/export.ts`, which also defines the browser-only `downloadCSV` (`document`/`Blob`/`URL`). This follows the existing `ics.ts` precedent (already imported by the server for `generateICS`, while carrying browser-only `downloadSingleICS`) and passes every script gate — but `tsc -p tsconfig.server.json` is already red from those DOM refs, and no script typechecks the server. Clean fix: split the browser-free parsers/formatters (`parseImportCSV`, ICS generation) into DOM-free modules (e.g. fold `parseImportCSV` into `src/utils/csv.ts`), keep download glue in browser-only files, then add a `typecheck:server` script and wire it into CI.
